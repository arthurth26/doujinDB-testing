...
const formatHandle = (handle:string, addAt: boolean = false): string => {
    const prefix = addAt? '@' : '';
    return handle.length < 10 ? `${prefix}${handle}`:`${prefix}${handle.slice(0,10)}...`
}

const platformRules: Record<string, (url: string, parts: string[]) => string> = {
    'youtube.com': (url: string, parts:string[]) => {
        const handle = parts[parts.length-1] || '';
        return handle.startsWith('@')? formatHandle(handle,false) : formatHandle(handle,true);
    },
    'twitter.com': (url: string, parts:string[]) => {
    const lastPart = parts[parts.length-1] || '';
    const match = lastPart.match(/^[^?,&]+/) || [lastPart]
    return formatHandle(match[0], false);
    },
    'x.com': (url: string, parts:string[]) => {
    const lastPart = parts[parts.length-1] || '';
    const match = lastPart.match(/^[^?,&]+/) || [lastPart];
    return formatHandle(match[0], false);
    },
    'instagram.com': (url: string, parts:string[]) => {
    const handle = parts[parts.length-1] || '';
    return formatHandle(handle,false);
    },
    'soundcloud.com': (url: string, parts:string[]) => {
    const handle = parts[parts.length-1] || '';
    return formatHandle(handle, false);
    },
    'facebook.com': (url: string, parts:string[]) => {
        const handle = parts[parts.length-1] || '';
        return formatHandle(handle, false)
    },
...
export const URLhandler = (url:string) => {
    const domain = url.match(/^(?:[a-z]+:\/\/)?(?:www\.)?([^\/]+)/i)?.[1] || '';
    const parts = url.split('/').filter((part)=> part)

    if (!domain || parts.length === 0 ) {return ''};

    for (const [platform, handler] of Object.entries(platformRules)) {
        if (domain.includes(platform)) {
            return handler(url,parts);
        }
    }
...
export const CircleContent = function CircleContent({circle, yearAndSeason}: {circle:circleData, yearAndSeason:string} ) {
        ...
        return (
            <li>
                <div id="circleContentContainer" className="relative p-2 min-h-55 rounded-xl bg-gray-700 text-gray-300" onClick={toggleDropDown}>
                    <h2 className="pb-2 font-semibold text-sm lg:text-base" > {parseInt(yearAndSeason.slice(0,4)) < 2025? `[${circle.realSp?.area? circle.realSp?.area:circle.webSp?.area}-${circle.realSp?.no? circle.realSp?.no:circle.webSp?.no}]`:`[${circle.area}-${circle.number}]`} {circle.name}</h2>
                                ...
                                {uniqueURLs && uniqueURLs.map((link, index) => { 
                                    {if (link.includes('youtube')) { return (
                                        <a href={link} className="cursor-pointer" target="_blank" rel="noopener noreferrer" key={index}>
                                            {isMobile?
                                            <li key={index} className="w-[40px] h-8 lg:w-full rounded bg-gray-600 hover:bg-gray-300 hover:text-gray-800 p-1">
                                            ...
                                            <li key={index} className="w-45 h-8 lg:w-full rounded bg-gray-600 hover:bg-gray-300 hover:text-gray-800 p-1">
                                        ...
                                        return (
                                            <a href={link} className="cursor-pointer" target="_blank" rel="noopener noreferrer" key={index}>
                                                ...
                                                <li key={index} className="w-45 lg:w-full rounded bg-gray-600 hover:bg-gray-300 hover:text-gray-800 p-1"> 
                                                    <div className="flex flex-row gap-3 items-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 50 50">
                                                        <path d="M 11 4 C 7.134 4 4 7.134 4 11 L 4 39 C 4 42.866 7.134 46 11 46 L 39 46 C 42.866 46 46 42.866 46 39 L 46 11 C 46 7.134 42.866 4 39 4 L 11 4 z M 13.085938 13 L 21.023438 13 L 26.660156 21.009766 L 33.5 13 L 36 13 L 27.789062 22.613281 L 37.914062 37 L 29.978516 37 L 23.4375 27.707031 L 15.5 37 L 13 37 L 22.308594 26.103516 L 13.085938 13 z M 16.914062 15 L 31.021484 35 L 34.085938 35 L 19.978516 15 L 16.914062 15 z"></path>
                                                    </svg>
                                        ...
                                        <a href={link} className="cursor-pointer" target="_blank" rel="noopener noreferrer" key={index}>
                                            {isMobile?
                                                ...
                                                <li key={index} className="w-45 lg:w-full rounded h-8 bg-gray-600 hover:bg-gray-300 hover:text-gray-800 p-1">
                                                    <div className="flex flex-row gap-3 items-center">
                                                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="25" height="25" viewBox="0 0 50 50">
                                                            <path d="M 30 11 C 26.398438 11 23 12.789063 21 15.6875 L 21 19.1875 L 21.3125 19.40625 L 21.6875 18.40625 C 23.085938 15.105469 26.40625 13 29.90625 13 C 34.90625 13 38.90625 17 38.90625 22 L 38.90625 24 L 40.40625 23.40625 C 41.105469 23.105469 41.800781 23 42.5 23 C 45.5 23 48 25.5 48 28.5 C 48 31.5 45.5 34 42.5 34 L 21 34 L 21 36 L 42.5 36 C 46.601563 36 50 32.601563 50 28.5 C 50 24.398438 46.601563 21 42.5 21 C 42 21 41.5 21.085938 41 21.1875 C 40.5 15.488281 35.800781 11 30 11 Z M 17 16 C 16.300781 16 15.601563 16.085938 15 16.1875 L 15 36 L 17 36 Z M 18 16 L 18 36 L 20 36 L 20 16.5 C 19.398438 16.300781 18.699219 16.101563 18 16 Z M 14 16.5 C 13.300781 16.800781 12.601563 17.101563 12 17.5 L 12 36 L 14 36 Z M 11 18.3125 C 10.199219 19.011719 9.5 19.90625 9 20.90625 L 9 36 L 11 36 Z M 6.5 22 C 6.324219 22.011719 6.148438 22.042969 6 22.09375 L 6 35.90625 C 6.300781 36.007813 6.699219 36 7 36 L 8 36 L 8 22.09375 C 7.699219 21.992188 7.300781 22 7 22 C 6.851563 22 6.675781 21.988281 6.5 22 Z M 5 22.3125 C 4.300781 22.511719 3.601563 22.8125 3 23.3125 L 3 34.6875 C 3.601563 35.085938 4.300781 35.488281 5 35.6875 Z M 2 24.09375 C 0.800781 25.394531 0 27.101563 0 29 C 0 30.898438 0.800781 32.605469 2 33.90625 Z"></path>
                                                        </svg>
                                        ...
                                        return (
                                            <a href={link} className="cursor-pointer" target="_blank" rel="noopener noreferrer" key={index}>
                                                { isMobile? 
                                                    ...
                                                    <li key={index} className="min-w-10 lg:w-full rounded bg-gray-600 hover:bg-gray-300 hover:text-gray-800 p-1">
                    ...
                    <div id="expandedContent" className={`transition-all duration-300 ${isDropDownOpen? 'max-h-[500px] overflow-y-auto': 'max-h-0 overflow-hidden'}`}>
...
export function ContentList() {
    ...
    return (
        <div className="pt-24">
            <ul className={`grid gap-2 grid-cols-1 lg:grid-cols-2 lg:p-2 2xl:grid-cols-3 ${isModalOpen? 'opacity-70':'opacity-100'}`}>
